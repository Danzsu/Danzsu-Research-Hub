...
import { assignIds, inlineText, safeHref, type Block, type BlockDraft, type Inline } from "../blocks.ts";
import { cleanDocument, filterNoise } from "./html-noise.ts";
import { imageUrl, type ImageResolver } from "./html-images.ts";
import { videoFromUrl } from "./util.ts";

// HTML → blocks: a DOM walk between the noise layers in html-noise.ts.
...
/** A <figure>: one image with its caption, nested figures with theirs, or panels plus a trailing caption paragraph. */
function pushFigure(el: Element, ctx: Ctx) {
