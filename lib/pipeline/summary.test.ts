...
/** Captures the prompt text sent in the single Gemini `generateContent` call the handler intercepts. */
...
test("summarize includes the extractionFailed note only when the flag is set (S1)", async () => {
...
test("writeNotes turns sections into heading and list blocks with ids (S1)", async () => {
  const restoreKey = withGeminiKey();
  const restore = mockFetch(async () => geminiResponse({ sections: [{ heading: "Findings", points: ["Point one", "Point two"] }] }));
  try {
